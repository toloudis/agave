...
void
prtyObject::AddProperty(shared_ptr<prtyPropertyUIInfo> i_pUIInfo)
{
  // When a property is assigned to this object, then this object
  // becomes the reference creator
  i_pUIInfo->AssignReferenceCreator(this);
...
//--------------------------------------------------------------------
void
prtyObject::RemoveProperty(shared_ptr<prtyPropertyUIInfo> i_pUIInfo)
{
...
prtyObject::DebugOutput()
{
  ...
  // for (i = 0; i < m_PropertiesInfo.GetNumberOfProperties(); ++i)
  {
    ...
    while (it != Uiil.end()) {
      shared_ptr<prtyPropertyUIInfo>& pPUII = (*it);
      if (pPUII != 0) {
        int i = 0;
...
//--------------------------------------------------------------------
// virtual
shared_ptr<prtyPropertyReference>
prtyObject::CreateReferenceForProperty(prtyProperty& i_Property)
{
  shared_ptr<prtyPropertyReference> static_reference(new StaticPropertyReference(&i_Property));
...
{
  shared_ptr<prtyPropertyReference> property_reference = this->CreateReferenceForProperty(i_Property);
