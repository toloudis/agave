...
namespace {
void
update_euler_from_quaternion(const maRotation& i_Quaternion, maVector3d& o_EulerAngles)
{
  i_Quaternion.GetEuler(o_EulerAngles.m_X, o_EulerAngles.m_Y, o_EulerAngles.m_Z);
}
void
update_quaternion_from_euler(const maVector3d& i_EulerAngles, maRotation& o_Quaternion)
{
  o_Quaternion.SetEuler(i_EulerAngles.m_X, i_EulerAngles.m_Y, i_EulerAngles.m_Z);
}
}
...
//--------------------------------------------------------------------
prtyRotation::prtyRotation(const std::string& i_Name, const maRotation& i_InitialValue)
  : prtyProperty(i_Name)
...
//----------------------------------------------------------------------------
void
prtyRotation::SetValue(const maRotation& i_Rotation, bool i_bDirty) // UndoFlags i_Undoable)
{
  this->SetQuaternion(i_Rotation, i_bDirty); // i_Undoable);
}
void
prtyRotation::SetQuaternion(const maRotation& i_Rotation, bool i_bDirty) // UndoFlags i_Undoable)
{
...
{
  maVector3d euler_angles(i_X, i_Y, i_Z);
...
{
  o_X = m_EulerAngles.m_X;
  o_Y = m_EulerAngles.m_Y;
  o_Z = m_EulerAngles.m_Z;
}
...
//--------------------------------------------------------------------
bool
prtyRotation::operator==(const maRotation& i_Value) const
{
...
prtyRotation::operator==(const prtyRotation& i_Value) const
{
  ...
  return ((m_EulerAngles - i_Value.m_EulerAngles).LengthSqr() < maConstants::c_fEpsilon);
}
bool
prtyRotation::operator!=(const prtyRotation& i_Value) const
{
...
// virtual
undoUndoOperation*
prtyRotation::CreateUndoOperation(shared_ptr<prtyPropertyReference> i_pPropertyRef)
{
